...
const generated = new Map<string, Video>();

export function saveVideo(video: Video): void {
  generated.set(video.id, video);
}

export function getVideo(id: string): Video | undefined {
  return generated.get(id) || MOCK.find((v) => v.id === id);
}

export function listVideos(): Video[] {
  const user = Array.from(generated.values()).sort(
    (a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
  );
  const mockIds = new Set(MOCK.map((m) => m.id));
  const merged = [...user, ...MOCK.filter((m) => !generated.has(m.id))];
